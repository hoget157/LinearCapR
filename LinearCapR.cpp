...
void LinearCapR::calc_outside(){
	for(int j = seq_n - 1; j >= 0; j--){
		...
		// O
		// O -> O
		update_sum(beta_O, j, (j + 1 < seq_n ? beta_O[j + 1] : 0) - energy_external_unpaired(j + 1, j + 1) / kT);
		
		// O -> O + S
		for(const auto [i, score] : alpha_S[j]){
			update_sum(beta_O, i, score + (j + 1 < seq_n ? beta_O[j + 1] : 0) - energy_external(i, j) / kT);
		}
		// cout << "O" << flush;
		...
		for(const auto [i, _] : alpha_SE[j]){
			// S -> SE
			if(i - 1 >= 0 && j + 1 < seq_n){
				// DUMP("S -> SE", i, j, get_value(beta_S, i - 1, j + 1));
		...
		for(const auto [i, _] : alpha_S[j]){
			// O -> O + S
			update_sum(beta_S, i, j, (i - 1 >= 0 ? alpha_O[i - 1] : 0) + (j + 1 < seq_n ? beta_O[j + 1] : 0) - energy_external(i, j) / kT);
			...
			// M2 -> S
			for(int n = 0; n <= /*MAXLOOP*/1000/*DEBUG*/ && j + n < seq_n; n++){
				// if(i == 50 && n == 0) DUMP("M2", n, i, j, get_value(beta_M2, i, j + n), - (energy_multi_bif(i, j) + energy_multi_unpaired(j + 1, j + n)) / kT);
